...
async function compareSelectedFavorites() {
  ...
  const fetches = selected.map(async key => {
    try {
      const res = await fetch(`wikipedia-proxy.php?details=${encodeURIComponent(key)}`);
...
async function openTankDetails(title, country) {
  ...
  try {
    const res = await fetch(`wikipedia-proxy.php?details=${encodeURIComponent(title)}`);
    const data = await res.json();
    ...
    const imageHtml = fields.image
      ? `<div class="stats-image"><img src="${fields.image}" alt="${data.title}" /></div>`
      : "";
...
async function loadTanks(q = "") {
  ...
  try {
    const res = await fetch(`db_search.php?q=${encodeURIComponent(trimmedQuery)}`);
    const data = await res.json();
    ...
    pages.forEach(page => {
      ...
      const thumbnail = page.thumbnail && page.thumbnail.url ? page.thumbnail.url : null;
